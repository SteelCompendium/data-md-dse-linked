...
effects:
  - effect: A creature can take the Ride move action only while mounted on another
      ...
      Ride move action to have their mount use the
      [Disengage](REL_PATH_PREFIXRules/Abilities/Common/Move%20Actions/DisengageREL_PATH_SUFFIX)
      move action as a free triggered action. A creature can use the Ride move
      action only once per round. A mounted creature can only have this move
      action applied to them once per round. This movement can be broken up with
      the rider's maneuver and main action however they wish.
```